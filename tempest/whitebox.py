...
class ComputeWhiteboxTest(test.ComputeFuzzClientTest, WhiteboxTest):
    ...
    def setUpClass(cls):
        ...
        cls.deploy_mode = cls.config.whitebox.openstack_deploy_mode
        cls.nova_dir = cls.config.whitebox.compute_source_dir
        cls.compute_bin_dir = cls.config.whitebox.compute_bin_dir
        cls.compute_config_path = cls.config.whitebox.compute_config_path
    ...
    def get_db_handle_and_meta(cls, database='nova'):
        ...
        global WHITEBOX_DB_AVAILABLE
        connection = None
        meta = None
        db_uri_map = {'nova': cls.config.whitebox.compute_db_uri,
                     'glance': cls.config.whitebox.glance_db_uri,
                     'keystone': cls.config.whitebox.keystone_db_uri,
                     'quantum': cls.config.whitebox.quantum_db_uri
                     }
        ...
        try:
            engine = create_engine(db_uri_map[database], **engine_args)
            connection = engine.connect()
            meta = MetaData()
            meta.reflect(bind=engine)

        except Exception:
            WHITEBOX_DB_AVAILABLE = False
